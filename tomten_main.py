import pygame
import time
import os
import sys
import RPi.GPIO as GPIO
import pigpio
import math
import threading
from time import sleep
def SetAngle(angle):
    duty = angle/18 + 2
    GPIO.output(23, True)
    pwm.ChangeDutyCycle(duty)
    sleep(1)
    GPIO.output(23, False)
    pwm.ChangeDutyCycle(0)
    return
def ease_sine(t):
    return 0.5* (1 - math.cos(math.pi * t))
def ease_in_out(t):
    if t>0.5:
        return 2 * t * t
    else:
        return -1 + (4 - 2 * t) * t

def SetLightOn(pin = 17):
    GPIO.output(pin, False)
def SetLightOff(pin = 17):
    GPIO.output(pin, True)

GPIO.setwarnings(False)
GPIO.setmode(GPIO.BCM)
GPIO.setup(23, GPIO.OUT)
GPIO.setup(17, GPIO.OUT)



# Initialize pygame mixer
pygame.mixer.init()
pygame.mixer.set_num_channels(10)  # Allow multiple sounds to play simultaneously

# Directory for sounds
SOUNDS_DIR = '/home/pi/tomtedorr_local/sounds'

# Mapping of commands to sounds and physical actions
ACTIONS = {
    'cat.run': {'sound': 'cat.run.ogg', 'physical': None},
    'cat.meaw': {'sound': 'cat.want_into.ogg', 'physical': None},
    'door.open': {'sound': 'door.open.ogg', 'physical': 'open'},
    'door.close': {'sound': 'door.close.ogg', 'physical': 'close'},
    'light.on': {'sound': 'silient.ogg', 'physical' : 'light.on'},
    'light.off': {'sound': 'silient.ogg', 'physical' : 'light.off'},
    'tomten.walk': {'sound': 'tomten.walk.ogg', 'physical': None},
    'guest.doorbell': {'sound': 'doorbell.ogg', 'physical': None},
    'guest.snowwalk': {'sound': 'outside.snowwalk.ogg', 'physical': None},
}

# Placeholder functions for physical door control
# Replace these with actual hardware control code, e.g., using RPi.GPIO for servo motor
def dummy_physical_action(duration = 0):
    sleep(duration)
def parse_sync(sync_str):
    """Parse the sync string like 'start+3', 'finish-3', etc."""
    if sync_str is None:
        return 'default', 0.0
    if '+' in sync_str:
        base, offset_str = sync_str.split('+')
        offset = float(offset_str)
    elif '-' in sync_str:
        base, offset_str = sync_str.split('-')
        offset = -float(offset_str)
    else:
        base = sync_str
        offset = 0.0
    return base, offset

def main(script_file):
    def DoorMove(start=1500, end = 700, duration = 0.3, pin = 23):
        steps = 40
        pwm = pigpio.pi()
        pwm.set_mode(23, pigpio.OUTPUT)
        pwm.set_PWM_frequency(23, 50)
        sleep(0.8)
        for i in range(steps + 1):
            t = i / steps
            fraction = ease_sine(t)
            pulse = start + fraction * (end - start)
            pwm.set_servo_pulsewidth(23, pulse)
            time.sleep(duration/steps)
        GPIO.output(pin, False)
        pwm.set_servo_pulsewidth(23, 0)
        pwm.stop()
    
        return
    SetLightOff()
    print(script_file)
    # Read the script file
    with open(script_file, 'r') as f:
        lines = [line.strip() for line in f if line.strip()]

    # List to hold actions with their start times and details
    action_list = []

    prev_start = 0.0
    prev_dur = 0.0
    for line in lines:
        parts = line.split()
        if not parts:
            continue
        cmd = parts[0]
        if cmd not in ACTIONS:
            print(cmd)
            print("Unknown command")
            continue

        sync_str = None
        if len(parts) >= 3 and parts[1] == '-S':
            sync_str = parts[2]

        base, offset = parse_sync(sync_str)
        sound_file = os.path.join(SOUNDS_DIR, ACTIONS[cmd]['sound'])
        if not os.path.exists(sound_file):
            print("Sound file not found:")
            continue

        snd = pygame.mixer.Sound(sound_file)
        dur = snd.get_length()
        

        if base == 'default':
            # Default: start after previous ends
            this_start = prev_start + prev_dur
        elif base == 'start':
            this_start = prev_start + offset
        elif base == 'finish':
            this_start = prev_start + prev_dur + offset - dur
        else:
            print("Unknown sync base: ")
            continue

        # Append to list
        action_list.append({
            'start_time': this_start,
            'snd': snd,
            'physical': ACTIONS[cmd]['physical'],
            'dur': dur
        })

        # Update previous
        prev_start = this_start
        prev_dur = dur

    if not action_list:
        print("No actions to perform.")
        return

    # Normalize start times to start from 0
    min_start = min(a['start_time'] for a in action_list)
    for a in action_list:
        a['start_time'] -= min_start
        if a['start_time'] < 0:
            a['start_time'] = 0  # Clamp to 0 if negative

    # Sort by start_time
    action_list.sort(key=lambda a: a['start_time'])
    print(action_list)
    # Execute
    start_abs = time.time()
    current_rel = 0.0

    for action in action_list:
        delay = action['start_time'] - current_rel
        if delay > 0:
            time.sleep(delay)
        print(action)
        thread_sound = threading.Thread(target = action['snd'].play())
        
        # Perform physical action if any
        print('there')
        if action['physical'] == 'open':
            thread_servo = threading.Thread(target = DoorMove(1500,700, 2, 23))
        elif action['physical'] == 'light.on':
            thread_servo = threading.Thread(target = SetLightOn())
        elif action['physical'] == 'light.off':
            thread_servo = threading.Thread(target = SetLightOff())
        elif action['physical'] == 'close':
            thread_servo = threading.Thread(target = DoorMove(700,1500, 0.33, 23))
        else:
            thread_servo = threading.Thread(target = dummy_physical_action())
        print('here')
        thread_sound.start
        thread_servo.start


        # Play sound

        current_rel = action['start_time']

    # Wait for the last sound to finish
    max_end = max(a['start_time'] + a['dur'] for a in action_list)
    remaining = max_end - current_rel
    if remaining > 0:
        time.sleep(remaining)
    print("Sequence completed.")

main("scenarious/door.txt")
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python script.py <txt_file>")
        sys.exit(1)
    main(sys.argv[1])
